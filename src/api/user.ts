...
export const getUsersByEmailApi = async (emails: string[]): Promise<GetUsersByEmailApiResponse> => {
  try {
    const { data } = await httpClient.post<GetUsersByEmailApiResponse>(`/users/emails`, {
      emails: emails,
    });
    return data;
  } catch (error: unknown) {
